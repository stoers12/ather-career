...
<div class="admin-project-grid"><?php foreach ($projects as $project): ?><article><div class="project-card-content"><h3><?php echo ownerEscapeHtml((string) $project['title']); ?></h3><p class="project-category"><?php echo ownerEscapeHtml((string) $project['category']); ?></p><p><?php echo ownerEscapeHtml((string) $project['description']); ?></p><div class="card-actions"><a class="project-edit" href="owner_projects.php?edit=<?php echo (int) $project['id']; ?>">Edit</a><form method="POST" action="owner_projects.php"><input type="hidden" name="action" value="delete"><input type="hidden" name="csrf_token" value="<?php echo ownerEscapeHtml(getCsrfToken()); ?>"><input type="hidden" name="id" value="<?php echo (int) $project['id']; ?>"><button type="submit" class="button-danger">Delete</button></form></div></div></article><?php endforeach; ?></div>
<?php ownerLayoutEnd();
